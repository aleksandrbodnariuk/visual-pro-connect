-- Fix user privacy by dropping and recreating functions with proper data protection

-- Drop existing functions first
DROP FUNCTION IF EXISTS public.get_safe_public_profiles();
DROP FUNCTION IF EXISTS public.get_safe_public_profiles_by_ids(uuid[]);
DROP FUNCTION IF EXISTS public.get_safe_user_profile(uuid);

-- Create minimal public profile function (only essential info)
...
-- Recreate safe public profiles with restricted data
...
-- Recreate by_ids version with restricted data
...
-- Create new function for detailed profile access (requires friendship or ownership)
...
-- Create privacy-conscious search function
