-- Seed the 4 configurable rep commission settings with current defaults
INSERT INTO public.site_settings (id, value, updated_at) VALUES
  ('rep-total-max-percent', '10', now()),
  ('rep-personal-percent', '5', now()),
  ('rep-manager-percent', '3', now()),
  ('rep-director-percent', '2', now())
ON CONFLICT (id) DO NOTHING;

-- Update process_order_profit to read configurable percentages from site_settings
  ...
  IF _order.representative_id IS NOT NULL THEN
    ...
    IF _rep IS NOT NULL THEN
      ...
      ELSIF _has_manager AND _has_director THEN
        _mgr_percent := _cfg_personal + _cfg_manager; _dir_percent := _cfg_director;
      ELSIF _has_rep THEN
        _rep_percent := _cfg_personal;
      ELSIF _has_manager THEN
        _mgr_percent := _cfg_personal + _cfg_manager;
      ELSIF _has_director THEN
        _dir_percent := _cfg_total_max;
      END IF;
