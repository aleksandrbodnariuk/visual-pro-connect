...
-- 1. Add remaining_qty to market table
ALTER TABLE public.market ADD COLUMN IF NOT EXISTS remaining_qty integer;

-- 2. Backfill remaining_qty from quantity for existing rows
UPDATE public.market SET remaining_qty = quantity WHERE remaining_qty IS NULL;

-- 3. Set NOT NULL after backfill
ALTER TABLE public.market ALTER COLUMN remaining_qty SET NOT NULL;

-- 4. Add default so new inserts auto-fill remaining_qty = quantity via trigger
CREATE OR REPLACE FUNCTION public.market_set_remaining_qty()
  RETURNS trigger
  LANGUAGE plpgsql
  SECURITY DEFINER
...
CREATE TRIGGER trg_market_set_remaining_qty
  BEFORE INSERT ON public.market
  FOR EACH ROW
  EXECUTE FUNCTION public.market_set_remaining_qty();

-- 5. Update RLS on market to allow all authenticated users to view active/partially_filled listings
DROP POLICY IF EXISTS "Shareholders can view active listings" ON public.market;
CREATE POLICY "Authenticated can view active listings"
  ON public.market FOR SELECT
  USING (
    (status IN ('active', 'partially_filled') AND auth.uid() IS NOT NULL)
    OR seller_id = auth.uid()
    OR is_user_admin(auth.uid())
  );

-- 6. Allow any authenticated user to create buy transactions (not just shareholders)
DROP POLICY IF EXISTS "Shareholders can create buy transactions" ON public.transactions;
CREATE POLICY "Authenticated users can create buy transactions"
  ON public.transactions FOR INSERT
  WITH CHECK (buyer_id = auth.uid() AND auth.uid() IS NOT NULL);

-- 7. Create server-side RPC for creating listings with overselling protection
CREATE OR REPLACE FUNCTION public.create_share_listing(
  _quantity integer,
...
BEGIN
  ...
  -- Check user has shareholder role
  ...
  -- Get reserved in open/partially_filled listings
  SELECT COALESCE(SUM(remaining_qty), 0) INTO _reserved
  FROM public.market
  WHERE seller_id = _user_id AND status IN ('active', 'partially_filled');
  ...
  -- Get price from company_settings
...
-- 8. Update approve_share_transaction to handle remaining_qty and partial fills
...
BEGIN
  ...
  -- Lock and check listing remaining_qty
  ...
  -- Check seller has enough actual shares
  ...
  IF seller_shares IS NULL OR seller_shares < tx.quantity THEN
    RAISE EXCEPTION 'Недостатньо акцій у продавця для завершення передачі' USING ERRCODE = 'check_violation';
  END IF;
  ...
  -- Update listing remaining_qty and status
  IF tx.share_id IS NOT NULL THEN
...
-- 9. Update reject to NOT change listing status (listing stays active)
...
BEGIN
  ...
  -- Just reject the transaction, listing stays as-is
