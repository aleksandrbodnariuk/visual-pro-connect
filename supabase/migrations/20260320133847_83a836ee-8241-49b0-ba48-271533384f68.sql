...
BEGIN
  ...
  -- STEP 2: Net profit = order_amount - remaining expenses
  ...
  IF _order.representative_id IS NOT NULL AND _net_profit > 0 THEN
    ...
    IF _rep IS NOT NULL THEN
      ...
      -- Determine percents by combination
  ...
  -- Shareholder pools (UNCHANGED: 50/20/17.5/12.5)
  ...
  -- Calculate unclaimed title bonuses
  -- 7 рівнів по 2.5% кожен = 17.5%
  -- Для кожного рівня: якщо немає акціонерів з відповідним титулом — бонус unclaimed
  IF _total_shares > 0 AND _net_after_reps > 0 THEN
    _level_pool := _net_after_reps * 0.025; -- 2.5% per level
    
    -- Levels: 1(Акціонер≥1%), 2(Магнат≥5%), 3(Барон≥10%), 4(Граф≥20%), 5(Маркіз≥30%), 6(Лорд≥40%), 7(Герцог≥50%)
    FOR i IN 1..7 LOOP
      _eligible_shares := 0;
    ...
    _unclaimed_title_bonus := GREATEST(0, _title_bonus_pool - _distributed_title_bonus);
  ELSE
    _unclaimed_title_bonus := _title_bonus_pool;
  END IF;

  -- STEP 4: Тільки unclaimed title bonuses йдуть в unallocated_funds
