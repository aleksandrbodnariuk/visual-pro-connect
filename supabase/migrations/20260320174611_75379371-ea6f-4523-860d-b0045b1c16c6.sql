...
BEGIN
  ...
  LOOP
    ...
    _sh_title_level := CASE
      WHEN _sh_percent >= 50 THEN 4
      WHEN _sh_percent >= 25 THEN 3
      WHEN _sh_percent >= 10 THEN 2
      WHEN _sh_percent >= 1  THEN 1
      ELSE 0
    END;
  ...
  FOR _level_pool IN
    SELECT unnest(ARRAY[
      ROUND(_title_bonus_pool * 0.10, 2),
      ROUND(_title_bonus_pool * 0.15, 2),
      ROUND(_title_bonus_pool * 0.25, 2),
      ROUND(_title_bonus_pool * 0.50, 2)
    ])
  LOOP
    NULL;
  END LOOP;
