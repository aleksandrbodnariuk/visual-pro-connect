...
-- 1. Create financial audit log table
CREATE TABLE public.financial_audit_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id uuid NOT NULL REFERENCES public.specialist_orders(id),
  net_profit numeric(12,2) NOT NULL DEFAULT 0,
  representatives_total numeric(12,2) NOT NULL DEFAULT 0,
  shareholders_total numeric(12,2) NOT NULL DEFAULT 0,
  unallocated_used numeric(12,2) NOT NULL DEFAULT 0,
  unallocated_added numeric(12,2) NOT NULL DEFAULT 0,
  created_at timestamp with time zone NOT NULL DEFAULT now()
);

-- 2. Indexes
CREATE INDEX idx_financial_audit_log_order_id ON public.financial_audit_log(order_id);
...
-- 4. Auto-insert from process_order_profit
...
BEGIN
  ...
  IF _order.representative_id IS NOT NULL THEN
    ...
    IF _rep IS NOT NULL THEN
      ...
      IF _has_rep AND _has_manager AND _has_director THEN _rep_percent := 0.05; _mgr_percent := 0.03; _dir_percent := 0.02;
      ELSIF _has_rep AND _has_manager THEN _rep_percent := 0.05; _mgr_percent := 0.03;
      ELSIF _has_manager AND _has_director THEN _mgr_percent := 0.08; _dir_percent := 0.02;
      ELSIF _has_rep THEN _rep_percent := 0.05;
      ELSIF _has_manager THEN _mgr_percent := 0.08;
      ELSIF _has_director THEN _dir_percent := 0.10;
      END IF;
  ...
  IF _total_shares > 0 AND _net_after_reps > 0 THEN
    ...
    FOR i IN 1..7 LOOP
      ...
      FOR _sh IN SELECT s.user_id, s.quantity FROM public.shares s WHERE s.quantity > 0 LOOP
        ...
        END IF;
        IF _sh_title_level >= i THEN _eligible_shares := _eligible_shares + _sh.quantity; END IF;
      END LOOP;
      IF _eligible_shares > 0 THEN _distributed_title_bonus := _distributed_title_bonus + _level_pool; END IF;
    END LOOP;
  ...
  -- Audit log entry
  INSERT INTO public.financial_audit_log (order_id, net_profit, representatives_total, shareholders_total, unallocated_used, unallocated_added)
  VALUES (_order_id, _net_profit, _rep_total_amount, _net_after_reps, _covered_from_fund, _unclaimed_title_bonus);

  _result := jsonb_build_object(
    'order_id', _order_id, 'order_amount', _order.order_amount, 'order_expenses', _order_expenses,
    'covered_from_fund', _covered_from_fund, 'remaining_expenses', _remaining_expenses,
    'net_profit', _net_profit, 'rep_total_amount', _rep_total_amount, 'net_after_reps', _net_after_reps,
    'specialists_pool', _specialists_pool, 'shares_pool', _shares_pool,
    'title_bonus_pool', _title_bonus_pool, 'admin_fund', _admin_fund,
    'distributed_title_bonus', _distributed_title_bonus, 'unclaimed_title_bonus', _unclaimed_title_bonus,
    'unallocated_snapshot', _unallocated_snapshot, 'unallocated_funds_after', _unallocated
  );
