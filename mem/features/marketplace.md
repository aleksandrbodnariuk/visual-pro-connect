...
**Етап 5 — Адмін-панель:** Нова вкладка «Маркетплейс» в адмінці (`AdminTabs` → `MarketplaceTab`) з двома підвкладками: 1) **Оголошення** (`MarketplaceListingsManager`) — список усіх оголошень із фільтром за статусом, пошуком за назвою/автором, можливістю змінити статус (active/reserved/sold/archived/draft), увімкнути/вимкнути VIP-буст вручну (кнопка корони), переглянути на сайті в новій вкладці та видалити з підтвердженням. RLS дозволяє адмінам делетити/апдейтити через `is_user_admin(auth.uid())`. 2) **Категорії** (`MarketplaceCategoriesManager`) — CRUD для `marketplace_categories` з вибором іконки lucide, перемикачем видимості, кнопками вгору/вниз для зміни `sort_order`, та автогенерацією slug-id з назви. Інвалідує кеш `marketplace_categories`, щоб публічний UI оновлювався миттєво.

**Етап 6 — VIP-відео в оголошенні:** Поле `marketplace_listings.video_url` (text, nullable) дозволяє VIP-користувачам додавати посилання на відео з YouTube/Instagram/TikTok/Facebook. У формі `MarketplaceNew` блок з полем введення показується лише для активних VIP (`useUserVip`); звичайні користувачі його не бачать. На сторінці деталей `MarketplaceListing` посилання обробляється через `extractVideoEmbed` (з `lib/videoEmbed.ts`) і рендериться в адаптивному iframe (вертикальне 9:16 для Shorts/Reels/TikTok, горизонтальне 16:9 для звичайного відео). Якщо платформа не розпізнана — показується клікабельне посилання.