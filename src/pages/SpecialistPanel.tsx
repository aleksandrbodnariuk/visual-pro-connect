...
import { Plus, CalendarDays, CheckCircle, Archive, Wallet } from 'lucide-react';
...
import { SpecialistPayouts } from '@/components/specialist/SpecialistPayouts';
        ...
        <Tabs value={mainTab} onValueChange={setMainTab}>
          <TabsList className="w-full grid grid-cols-2 mb-4">
            <TabsTrigger value="orders" className="gap-1 text-xs sm:text-sm">
              <CalendarDays className="h-3.5 w-3.5 hidden sm:block" /> Замовлення
            </TabsTrigger>
            <TabsTrigger value="payouts" className="gap-1 text-xs sm:text-sm">
              <Wallet className="h-3.5 w-3.5 hidden sm:block" /> Виплати
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="payouts">
            {user && <SpecialistPayouts userId={user.id} />}
          </TabsContent>
        </Tabs>
      </div>
