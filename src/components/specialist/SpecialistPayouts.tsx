...
export function SpecialistPayouts({ userId }: Props) {
    ...
    if (!error && data) {
      ...
      if (orderIds.length > 0) {
        ...
        if (orders) {
          ...
          orders.forEach((o: any) => { titles[o.id] = o.title; });
  ...
  const handleConfirm = async () => {
    ...
    try {
      const { error } = await supabase.rpc('confirm_spec_payout', {
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            Виплат ще немає. Вони з'являться після розподілу адміністратором.
          </div>
        </CardContent>
      </Card>
    );
        ...
        <DialogContent>
          ...
          {confirmDialog && (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Замовлення:</span> {orderTitles[confirmDialog.order_id] || confirmDialog.order_id}</div>
              {confirmDialog.role_at_calculation && (
                <div className="flex justify-between"><span className="text-muted-foreground">Роль:</span> {confirmDialog.role_at_calculation}</div>
              )}
